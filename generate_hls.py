...
class CDSE:
    def __init__(self, hardware_constraints):
        ...
        hardware_constraints = {
            "total_dsp": 8000,
            "total_bram": 2000,   # BRAM blocks (36Kb)
            "total_uram": 512,    # URAM blocks (288Kb)
            "total_hbm_channels": 32,
            "hbm_bw_per_channel": 32e9,  # Bytes/s (≈ 32 GB/s)
            "dsp_frequency": 1.0e9       # Hz
        }
        """
        self.constraints = hardware_constraints
        ...
        for tile_m, tile_n, tile_k in tile_candidates:
            ...
            if (dsp_required <= self.constraints["total_dsp"] and
                ...
                designs.append({
                    ...
                    "tile": [tile_m, tile_n, tile_k],
                    "dsp": dsp_required,
                    "bram": mem_required["bram"],
                    "uram": mem_required["uram"],
                    "hbm_channels": hbm_channels,
                    "throughput_GFLOPS": throughput,
                    "efficiency": efficiency
                })

        return pd.DataFrame(sorted(designs, key=lambda x: x["throughput_GFLOPS"], reverse=True))
    ...
    def calculate_hbm_channels(self, tile_m, tile_n, tile_k):
        """Estimate HBM channels required"""
        # Data per tile (Bytes)
        data_volume = (tile_m * tile_k + tile_k * tile_n + tile_m * tile_n) * 4
        # Bandwidth demand = data_volume per cycle * freq
    ...
    def calculate_memory(self, tile_m, tile_n, tile_k):
        """Estimate BRAM/URAM usage"""
        # Xilinx: BRAM = 36Kb = 4.5 KB, URAM = 288Kb = 36 KB
        bram_bytes = (tile_m * tile_k + tile_k * tile_n) * 4
        uram_bytes = (tile_m * tile_n) * 4
        return {
            "bram": int(np.ceil(bram_bytes / 4608)),   # per BRAM block
            "uram": int(np.ceil(uram_bytes / 36864))   # per URAM block
        }

    def estimate_throughput(self, M, K, N, dsp_count):
        """Throughput (GFLOPS) and efficiency"""
        ops = 2 * M * K * N
